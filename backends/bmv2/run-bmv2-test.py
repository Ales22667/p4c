...
PARSER.add_argument("-p", "--usePsa", dest="use_psa", action="store_true", help="Use psa switch")
PARSER.add_argument(
    "-bd",
    "--buildir",
    dest="builddir",
...
class Options:
    ...
    switch_target_specific_options: List[str] = []
    hasBMv2: bool = False  # Is the behavioral model installed?
    usePsa: bool = False  # Use the psa switch behavioral model?
    runDebugger: str = ""
    # Log packets produced by the BMV2 model if path to log is supplied
    observationLog: Optional[Path] = None
...
def process_file(options: Options) -> int:
    ...
    for opt in options.compiler_options:
        cmd += f" {opt}"
    for opt in options.switch_target_specific_options:
        cmd += f" {opt}"
    ...
    if options.runDebugger:
        ...
        cmd = options.runDebugger + cmd
        os.execvp(cmd[0], cmd)
    result = testutils.exec_process(cmd, timeout=30)
    returnvalue = result.returncode
    if returnvalue != testutils.SUCCESS:
        testutils.log.info("Error compiling")
...
def create_options(test_args: Any) -> Optional[Options]:
    ...
    if not testdir:
        print("No test directory provided. Generating temporary folder.")
    ...
    if test_args.builddir:
        options.compiler_build_dir = test_args.builddir
    else:
    ...
    for switch_option in test_args.switch_options:
        options.switch_target_specific_options.extend(switch_option.split())
    for init_cmd in test_args.init_cmds:
        options.init_commands.append(init_cmd)
    options.usePsa = test_args.use_psa
    if test_args.pp:
    ...
    options.observationLog = test_args.obs_log
    ...
    if not options.observationLog:
        if options.testName:
            options.observationLog = Path(options.testName).with_suffix(".p4.obs")
        else:
            basename = os.path.basename(options.p4_file)
            base, _ = os.path.splitext(basename)
            dirname = options.p4_file.parent
            options.observationLog = dirname.joinpath(f"{base}.p4.obs")
    return options
